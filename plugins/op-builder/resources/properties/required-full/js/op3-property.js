...
;(function($, window, document) {
     ...
     * @param {Class}
     */
    OP3.Elements._extension.prop.RequiredFull = OP3.defineClass({

        Name: "OP3.Property.RequiredFull",
        ...
        Prototype: {
            ...
            _defaults: {
                label: function() {
                    return OP3._("Required Full");
                },
                hidden: true,
            },

            getter: function (media) {
                var result = this.element.getOption("required", media);
                if (result === "1")
    ...
    OP3.bind("elementchange::*::required", function(e, o) {
        var element = OP3.$(o.node).element() || OP3.Document;
        var prop = element.findProperty(o.id + "Full");
        if (!prop)
            return;

        var valueBefore = o.value.before ? 'required="' + o.value.before + '"' : "";
        var valueAfter = o.value.after ? 'required="' + o.value.after + '"' : "";
